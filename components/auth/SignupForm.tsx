...
export default function SignupForm({ onSwitch, onBack }: { onSwitch: () => void; onBack: () => void }) {
  ...
  return (
    <div className="w-full max-w-md">
      <button className="text-sm text-slate-500 hover:text-slate-600 inline-flex items-center gap-1 mb-6" onClick={onBack}>
          ...
          return (
            <div key={s} className="flex items-center gap-2 flex-1">
              <div className={`w-7 h-7 rounded-full grid place-items-center text-xs font-semibold ${
                done ? "bg-success text-white" : active ? "bg-primary text-white" : "bg-slate-100 text-slate-500"
              }`}>{i + 1}</div>
              {i < 2 && <div className={`flex-1 h-0.5 ${done ? "bg-success" : "bg-slate-200"}`} />}
            </div>
          );
        })}
      </div>
      ...
      {step === "conta" && (
        <>
          ...
          <p className="text-sm text-slate-400 mt-1">Vamos começar pelo básico</p>
      ...
      {step === "contas-bancarias" && (
        <>
          ...
          <p className="text-sm text-slate-400 mt-1">Adicione onde você guarda seu dinheiro (você pode pular)</p>
          <div className="mt-6 space-y-3">
            {contas.map((c, i) => (
              <div key={i} className="border border-slate-200 rounded p-3 flex items-center gap-3">
                ...
                <div className="flex-1 text-sm">
                  ...
                  <div className="text-xs text-slate-500">{c.banco} · R$ {c.saldo.toLocaleString("pt-BR")}</div>
                </div>
      ...
      {step === "cartoes" && (
        <>
          ...
          <p className="text-sm text-slate-400 mt-1">Adicione seus cartões para controlar o limite</p>
          <div className="mt-6 space-y-3">
            {cartoes.map((c, i) => (
              <div key={i} className="border border-slate-200 rounded p-3 flex items-center gap-3">
                ...
                <div className="flex-1 text-sm">
                  <div className="font-medium">{c.nome}</div>
                  <div className="text-xs text-slate-500">{c.banco} · R$ {c.limite.toLocaleString("pt-BR")} · venc. dia {c.dia}</div>
                </div>
      ...
      {step === "fim" && (
        <div className="text-center py-12">
          ...
          <p className="text-sm text-slate-400 mt-2">Carregando seu painel...</p>
        </div>
      )}
